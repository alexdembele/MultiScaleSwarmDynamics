breed [centroids centroid] ;; agent corresponding to centroids
breed [birds bird]         ;; basic agent
...
centroids-own
[
  idcentroid       ;; identity of the centroid
  poids            ;; number of turtle in the swarm corresponding to the centroid
]

globals            ;; mostly information to send to the parent model
[i
currentId
  max-id
  nbId
...
to go
  ...
  [clear-links ;clear all the swarm identifier to recompute the swarm detection
   ...
   set i 0
    ...
    color-swarm      ;; color the agent based on the swarm they belong to

    let headx mean [sin heading] of birds
    let heady mean [cos heading] of birds


]

  count-unique-ids      ;;compute id of agent to deal with swarm
  calculate-centroid    ;; compute the centroid of a swarm

  set varianceheading lput variance ([heading] of birds) varianceheading
...
to print-heading ;; display a list containing the heading of aeach agent
  set listHead []
  ask birds
  [
    set listHead lput heading listHead
    set listHead lput "," listHead
  ]
  ;print(listHead)
  print(varianceheading)
...
to detect-swarm    ;;swarm detection
  ask birds[
  ...
  let y 0
  repeat 300   ;;propagate id through links
  [
...
to count-unique-ids ;; count id to send to parent
...
to calculate-centroid   ;;calculate swarm centroids with id
  set listXcentroid []
  set listYcentroid []
  ...
  if any? matchingBirds [
    ...
    create-centroids 1 [   ;;creation of agent centroid
